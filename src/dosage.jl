...
function alt_allele_dosage!(buf::Vector{T}, p::Pgen, v::Variant; 
    genotypebuf::Union{Nothing, Vector{UInt8}}=nothing,
    genotypeprevbuf::Union{Nothing, Vector{UInt8}}=nothing
    ) where T <: AbstractFloat
    n_samples = p.header.n_samples
    if genotypebuf === nothing 
        genotypebuf = Vector{UInt8}(undef, n_samples)
    end
    _, variant_record, offset = get_genotypes!(genotypebuf, p, v; prev_buf=genotypeprevbuf)

    # convert genotype to floating-point values
    basemap = T == Float32 ? basemap_Float32 : basemap_Float64
    @inbounds for i in 1:n_samples
        buf[i] = basemap[genotypebuf[i] + 1]
    end
    ...
    if v.record_type & 0x10 != 0 # skip phase info (aux track 2) 
        offset = _phase_offset(p, genotypebuf, variant_record, offset)
    end
...
function ref_allele_dosage!(buf::Vector{T}, p::Pgen, v::Variant; 
    genotypebuf::Union{Nothing, Vector{UInt8}}=nothing,
    genotypeprevbuf::Union{Nothing, Vector{UInt8}}=nothing
    ) where T <: AbstractFloat
    _, offset = alt_allele_dosage!(buf, p, v; genotypebuf=genotypebuf, genotypeprevbuf=genotypeprevbuf)
